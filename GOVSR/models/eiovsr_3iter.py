...
class SimpleUpscale(nn.Module): 
    def __init__(self, basic_feature=64, scale=4, act=nn.LeakyReLU(0.1,True)):
        ...
        self.scale = scale
        self.act = act
        self.upscale = UPSCALE(basic_feature * 4, scale, act)
...
class UNIT(nn.Module): 
    def __init__(self, kind='successor', basic_feature=64, mid_c = 114, num_channel=3, num_b=5, num_feature=1, alignment_filter = 144, scale=4, act=nn.LeakyReLU(0.1,True), lnl=None, head=None, merge=None, upscale = None):
    ...
    def forward(self, lqs, feats, flows_forward, flows_backward, mode):
        ...
        for idx, t in enumerate(frame_idx):
            ...
            # hts = [feats[k][t] for k in keys[:-1]] + [ht_past, ht_future_2current, lq_current, lq_past, lq_future]
            # feat = [ht_current] + [
            #     feats[k][idx]
...
class Net(nn.Module):
    def __init__(self, config):
        ...
        self.mode = ['backward_0', 'forward_0', 'backward_1']
        if hasattr(config.model, 'mode'):
            self.mode = config.model.mode
        
        print(self.mode)
        self.units = []
        self.units.append(UNIT('precursor', self.bf, self.rb_filter, 3, 2, 0, self.alignment_filter, self.scale, self.act, self.lnl, self.head, self.merge, self.upscale))
        self.units.append(UNIT('successor', self.bf, self.rb_filter, 3, 2, 1, self.alignment_filter, self.scale, self.act, self.lnl, self.head, self.merge, self.upscale))
        self.units.append(UNIT('successor', self.bf, self.rb_filter, 3, 2, 2, self.alignment_filter, self.scale, self.act, self.lnl, self.head, self.merge, self.upscale))
